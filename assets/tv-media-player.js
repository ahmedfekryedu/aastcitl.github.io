(function(root) {
  'use strict';
  let generation = 0, active, activeRow, ready = true, preparedVideo, stalledAt = 0, playbackFailure;
  ...
  function requestAudioUnlock() {
    let button = document.getElementById('tv-enable-audio');
    if (button) return;
    button = document.createElement('button'); button.id = 'tv-enable-audio';
    button.textContent = 'اضغط لتفعيل الصوت';
    button.style.cssText = 'position:fixed;bottom:110px;right:20px;z-index:99999;background:#F3A628;color:#172150;border-radius:12px;padding:10px 18px;font:700 16px Cairo,sans-serif';
    button.addEventListener('click', () => { button.remove(); root.dispatchEvent(new Event('citl-tv-unlock-audio')); updateSettings(activeRow); });
  ...
  function updateSettings(row) {
    ...
    activeRow = row;
    active.style.objectFit = row.fit_mode === 'contain' ? 'contain' : 'cover';
    if (active.tagName === 'VIDEO' && active.muted === !!row.play_sound) {
      if(active.ended)active.muted=!row.play_sound;
      else play(active, !!row.play_sound).catch(() => {});
    }
    if (!row.play_sound) document.getElementById('tv-enable-audio')?.remove();
  }
  ...
  function cancel(discardPreload = false) {
    ...
    document.getElementById('tv-enable-audio')?.remove(); releasePrevious();
  ...
  async function show(row, card, host, container, onVisible, onFailure) {
    ...
    try {
      ...
      if (video(row)) await play(media, false);
      if (ticket !== generation) { media.pause?.(); return; }
      host.replaceChildren(media); container.replaceChildren(card); active = media; activeRow = row; stalledAt = 0;
      releasePrevious();
      playbackFailure = () => {
      ...
      if (video(row) && row.play_sound) await play(media, true);
      if (ticket !== generation) return;
