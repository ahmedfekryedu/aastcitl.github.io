(function () {
  ...
  async function validateProtectedPage() {
    ...
    try {
      ...
      document.querySelectorAll('[data-manager-only]').forEach((el) => el.classList.toggle('hidden', !CITLRuntime.can(safeUser,'full')));
    } catch {
  ...
  function renderNotifications(items) {
    ...
    for (const item of items) {
      ...
      button.addEventListener('click', async () => {
        ...
        if (CITLRuntime.localURL(item.route)) location.href = CITLRuntime.localURL(item.route);
  ...
  const protectedPageReady = validateProtectedPage();
  addEventListener('load', async () => {
    await protectedPageReady;
    ...
    if(location.pathname.startsWith('/schedules/')&&new URLSearchParams(location.search).get('open')==='my-schedule')await window.openMyScheduleModal?.();
    const meetingId=new URLSearchParams(location.search).get('meeting');
    if(meetingId&&typeof loadMeetings==='function'){await loadMeetings();const selected=typeof meetings!=='undefined'?meetings.find(x=>String(x.id)===meetingId):null;if(selected&&typeof showMeetingDetails==='function')showMeetingDetails(selected);else window.showNotification?.('الحجز غير موجود أو لم يعد متاحًا لحسابك','error');}
    ...
    await checkUserNotifications();
    setInterval(()=>{if(!document.hidden)checkUserNotifications();},60000);
    try {
      const current = await session();
      window.sb.channel('citl-schedule-revision').on('postgres_changes',{event:'*',schema:'public',table:'site_settings',filter:'setting_key=eq.schedule_revision'},async()=>{
        try{await window.refreshCache?.();if(location.pathname.startsWith('/schedules/')){const user=CITLRuntime.get('currentUser',{});if(document.getElementById('my-schedule-modal')?.classList.contains('flex'))await window.showMyScheduleView?.(user.linked_instructors||[]);await window.applyFilters?.();}await window.updateDashboardCardsOnly?.();}catch(error){console.warn('schedule refresh',error);}
      }).subscribe();
      if (window.__citlNotificationChannel) return;
      window.__citlNotificationChannel = window.sb.channel(`notifications:${current.user.id}`).on('postgres_changes', {
        event: '*', schema: 'public', table: 'citl_notifications', filter: `recipient_id=eq.${current.user.id}`
      }, async (payload) => {
        await checkUserNotifications();
        if (payload?.eventType === 'INSERT' && !document.getElementById('notif-list')) {
          window.showNotification?.(payload.new?.title || 'وصلك تنبيه جديد', 'success');
        }
      }).subscribe();
    } catch (error) { console.error('notification realtime:', error); }
  });
  document.addEventListener('visibilitychange', () => { if (!document.hidden) checkUserNotifications(); });
})();
