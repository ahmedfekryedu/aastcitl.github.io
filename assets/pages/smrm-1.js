...
async function initializeApp() {
    try {
        ...
        function initializeCalendar() {
            currentWeekStart = window.CITLAcademicBooking.weekStart();
            updateWeekDisplay();
            renderCalendar();
        }
...
function updateWeekDisplay() {
    ...
    for (let i = -3; i <= 3; i++) {
        ...
        weekEndDisplay.setDate(weekEndDisplay.getDate() + 5);
...
function renderCalendar() {
    window.CITLAcademicBooking.load(supabase,currentRoomId,currentWeekStart,renderCalendar);
    ...
    const dates = window.CITLAcademicBooking.weekDays(currentWeekStart);
    ...
    for (let i = 0; i <= dates.length; i++) {
        ...
        if (i === 0) {
             ...
             cell.dataset.calendarDate = window.CITLAcademicBooking.dateKey(dateObj);
             cell.dataset.calendarHeader = "true";
...
cell.className = 'time-slot metallic-blue-fill random-slanted-lines text-white font-bold flex flex-col items-center justify-center tracking-wide border-l border-blue-800 py-1';             cell.innerHTML = `
                <span class="text-xs md:text-sm font-extrabold mb-0.5">${dayNames[dateObj.getDay()]}</span>
    ...
    timeSlots.forEach(time => {
        ...
        for (const currentDate of dates) {
            ...
            cell.dataset.calendarDate = window.CITLAcademicBooking.dateKey(currentDate);
            cell.dataset.calendarTime = time;
            ...
            const meeting = meetings.find(m => {
                ...
                return meetingDate.toDateString() === currentDate.toDateString() && 
                       timeToMinutes(m.startTime) < timeToMinutes(time) + 60 && timeToMinutes(m.endTime) > timeToMinutes(time) &&
                       ['confirmed','modification_requested','cancellation_requested'].includes(m.status);
            });
            ...
            if (meeting) {
                ...
                window.CITLAcademicBooking.paint(cell,supabase,currentRoomId,currentWeekStart,currentDate,time,() => openAddMeetingModal(currentDate,time));
            }
            container.appendChild(cell);
        }
    });
...
function checkConflicts(newMeeting, excludeId = null) {
            const conflictingMeeting = meetings.find(meeting => {
                ...
                if (!meeting || !meeting.startTime || !meeting.endTime || ['rejected','cancelled'].includes(meeting.status)) return false;
...
function jumpToWeek(dateISO) {
    ...
    // حساب بداية أسبوع الحجز (السبت)
    currentWeekStart = window.CITLAcademicBooking.weekStart(newDate);
