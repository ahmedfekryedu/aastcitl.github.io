(function (root) {
  ...
  async function upload(sb, file, progress, stage = () => {}) {
    const mediaType = validate(file);
    stage('جاري فحص الملف وتجهيز الرفع…');
    if (mediaType === 'video') await validateVideo(file);
  ...
  async function save(sb, {old, metadata, file, progress = () => {}, stage = () => {}}) {
    if (!old && !file) throw new Error('اختر ملف الإعلان أولًا');
    const fresh = file ? await upload(sb, file, progress, stage) : null;
