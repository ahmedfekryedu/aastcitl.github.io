  ...
  async function convert(file, {stage = () => {}, signal} = {}) {
    ...
    try {
      ...
      const args = ['-hide_banner','-i','/input/source','-map',`0:${video.index}`,'-map','0:a:0?','-t',String(duration),
        '-vf',"scale=w='max(2,trunc(iw*sar/2)*2)':h='max(2,trunc(ih/2)*2)',setsar=1,scale=w='max(2,trunc(iw*min(1,max(1080/iw,1920/ih))/2)*2)':h='max(2,trunc(ih*min(1,max(1080/iw,1920/ih))/2)*2)',crop=w='min(iw,1080)':h='min(ih,1920)',pad=1080:1920:(ow-iw)/2:(oh-ih)/2:color=0x172150,fps=30",
