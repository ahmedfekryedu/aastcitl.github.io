(function(){
  ...
  const KEY='eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InhncXVrZGJvbnp1a3hycGpvdm1iIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NjM4OTg4MDMsImV4cCI6MjA3OTQ3NDgwM30.3-70d7uB-zjVF7Jfr8ZjITT7suYPo3EWsYngO-sFVqM';
  const sb=window.supabase.createClient(URL,KEY,{global:{fetch:window.CITLReadFetch},auth:{storageKey:'sb-main-auth',persistSession:true,autoRefreshToken:true}});
  ...
  async function api(action,payload={}){
    ...
    return window.CITLRequests.rpc(sb,'citl_secure_api',{p_action:action,p_payload:payload});
  }
  ...
  function defaultTermId(){return (state.terms||[]).find(t=>t.is_active&&!t.schedule_deleted_at)?.id||(state.terms||[]).find(t=>!t.schedule_deleted_at)?.id||''}
  ...
  function fillSelects(){
    ...
    for(const id of ['visitor-term','qr-term','reports-term']){
      ...
      (state.terms||[]).filter(t=>id==='reports-term'||!t.schedule_deleted_at).forEach(t=>q.append(option(t.id,`${t.name}${t.is_active?' (نشط)':''}${t.schedule_deleted_at?' (سجل سابق)':''}`)));
  ...
  function renderReports(){
    ...
    const presenceRows=(state.presence_sessions||[]).filter(s=>profileIds.has(s.user_id)&&(!term||((s.started_at||'').slice(0,10)>=term.starts_on&&(!term.ends_on||(s.started_at||'').slice(0,10)<=term.ends_on)))).map(s=>[
  ...
  async function qrRequest(name,args){
    const controller=new AbortController(),timer=setTimeout(()=>controller.abort(),15000);
    try{const {data,error}=await sb.rpc(name,args).abortSignal(controller.signal);if(error){if(error.code==='PGRST202'||error.code==='42883')throw new Error('طبّق ملف SUPABASE-QR-RESTORE.sql لحفظ واستعادة QR');throw error}return data}
    finally{clearTimeout(timer)}
  }
  async function renderSavedQr(){
  ...
  async function load(){
    try{state=await api('management.snapshot');render();await renderSavedQr()}
  ...
  document.getElementById('qr-term').addEventListener('change',()=>{renderQrEligibility();renderSavedQr()});
  ...
  window.CITLScheduleImportUI.init(sb);
  if(new URLSearchParams(location.search).get('manage')==='schedules'){document.documentElement.classList.add('schedule-embedded');}
  else load();
})();
