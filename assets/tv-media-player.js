(function(root) {
  ...
  async function show(row, card, host, container, onVisible, onFailure) {
    ...
    const source = isVideo ? row.image_url : await imageSource(row.image_url);
    ...
    try {
      await new Promise((resolve, reject) => {
        timer = setTimeout(
          () => reject(new Error('تعذر تحميل الإعلان')),
          25000
        );
        ...
        media.addEventListener(
          'error',
          () => reject(new Error('ملف الإعلان غير متاح')),
          { once: true }
        );
      ...
      if (isVideo && (!Number.isFinite(media.duration) || media.duration <= 0)) {
      ...
      ready = true;
      onVisible(isVideo ? media.duration : null);
  ...
  function preload(row) {
    ...
    imageSource(row.image_url).catch(() => {});
  }
  ...
  function videoProgress() {
    ...
    if (!active.ended && active.readyState < 3) {
    ...
    if (
      active.error ||
      (stalledAt && Date.now() - stalledAt > 30000)
    ) {
      playbackFailure?.();
      return null;
    }
