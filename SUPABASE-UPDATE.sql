...
drop index if exists public.notifications_recipient_dedupe_uq;
...
begin
  foreach t in array array[
    ...
    'visiting_lecture_sessions','attendance_scans','security_audit_log'
  ] loop
...
begin
  foreach t in array array[
    ...
    'presence_events','schedule_actions','academic_schedule','faculty_directory','exam_schedule',
...
drop policy if exists smrm_reservations_authenticated_read on public.smrm_reservations;
create policy smrm_reservations_authenticated_read on public.smrm_reservations for select to authenticated using (true);
drop policy if exists smrm_reservations_manager_write on public.smrm_reservations;
create policy smrm_reservations_manager_write on public.smrm_reservations for all to authenticated using (public.is_manager()) with check (public.is_manager());
grant select, insert, update, delete on public.smrm_reservations to authenticated;
create or replace view public.smrm_reservations_public with (security_invoker = false)
as select date, start_time, end_time, status from public.smrm_reservations where status = 'confirmed';
revoke all on public.smrm_reservations_public from public;
grant select on public.smrm_reservations_public to anon, authenticated;
...
begin
  if tg_op = 'INSERT' then
    for manager_row in select id from public.profiles where role = 'manager' and id <> new.user_id loop
      insert into public.notifications(recipient_id, actor_id, kind, title, body, entity_type, entity_id, route, dedupe_key)
  ...
  foreach table_name in array array['settings','site_settings'] loop
    ...
    for row_json in execute format('select to_jsonb(t) from public.%I t', table_name) loop
      for pair in select * from jsonb_each_text(row_json) loop
        if pair.key ~* '(campus|college|academy).*(ip|cidr)|(ip|cidr).*(campus|college|academy)' then
          begin
...
begin
  ...
  if p_action='notifications.list' then
    select coalesce(jsonb_agg(to_jsonb(n) order by n.created_at desc),'[]'::jsonb) into result
      from (select * from public.notifications where recipient_id=uid and archived_at is null order by created_at desc limit 80) n;
    return jsonb_build_object('ok',true,'notifications',result);
  elsif p_action='notifications.read' then
    update public.notifications set read_at=coalesce(read_at,now()) where recipient_id=uid and read_at is null
      and (not (p_payload ? 'ids') or id in (select value::uuid from jsonb_array_elements_text(p_payload->'ids')));
    ...
    dept := coalesce(
      ...
      (select v.department_key from public.visiting_lecturers v where v.normalized_name=public.citl_normalize_arabic_name(lecture->>'instructor') limit 1),
    ...
    loop
      insert into public.notifications(recipient_id,actor_id,kind,title,body,entity_type,entity_id,route,payload,dedupe_key,read_at)
  ...
  if p_action='registration.manager_ticket' then
    ...
    if matched_visitor_id is null then
      ...
      values(left(trim(input->>'full_name'),180),public.citl_normalize_arabic_name(input->>'full_name'),input->>'department_key',selected_profile_id,
        string_to_array(coalesce(input->>'aliases',''),','),coalesce((input->>'is_active')::boolean,true),uid) returning to_jsonb(visiting_lecturers) into row_json;
    else
      ...
      update public.visiting_lecturers set full_name=left(trim(input->>'full_name'),180),normalized_name=public.citl_normalize_arabic_name(input->>'full_name'),department_key=input->>'department_key',profile_id=nullif(input->>'profile_id','')::uuid,
        ...
        is_active=coalesce((input->>'is_active')::boolean,true),updated_at=now() where id=matched_visitor_id returning to_jsonb(visiting_lecturers) into row_json;
    end if;
    if previous_profile_id is not null and previous_profile_id is distinct from selected_profile_id
       and not exists(select 1 from public.visiting_lecturers v where v.profile_id=previous_profile_id and v.is_active) then
      update public.profiles set is_visiting_lecturer=false where id=previous_profile_id;
    end if;
    if row_json->>'profile_id' is not null then
      update public.profiles set is_visiting_lecturer=coalesce((row_json->>'is_active')::boolean,false) where id=(row_json->>'profile_id')::uuid;
    end if;
    if exists(select 1 from public.academic_terms where is_active) then
      perform public.citl_sync_visiting_sessions(null);
    end if;
    return jsonb_build_object('ok',true,'data',row_json);
    ...
    insert into public.department_heads(department_key,profile_id,active,assigned_by,assigned_at) values(input->>'department_key',(input->>'profile_id')::uuid,true,uid,now())
...
declare
  ...
  lecture_day text:=trim(to_char(timezone('Africa/Cairo',now()),'Day'));
  matched record;
  match_count integer;
...
begin
  ...
  if not coalesce((select is_visiting_lecturer from public.profiles where id=uid),false) then raise exception 'تسجيل الحضور متاح للمنتدبين المسجلين فقط'; end if;
  select * into visitor from public.visiting_lecturers where profile_id=uid and is_active limit 1;
  if not found then raise exception 'حسابك غير مربوط بسجل منتدب نشط'; end if;
...
begin
  ...
  if not coalesce(profile.is_visiting_lecturer,false) or not exists(select 1 from public.visiting_lecturers where profile_id=uid and is_active) then
...
begin
  foreach required_table in array array[
    'notifications','department_heads','academic_terms','visiting_lecturers',
    'room_qr_tokens','visiting_lecture_sessions','attendance_scans','security_audit_log',
  ...
  foreach required_table in array array[
    'notifications','visiting_lecturers','attendance_scans','security_audit_log',
    'campus_networks','registration_tickets'
  ] loop
