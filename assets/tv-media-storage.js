(function (root) {
  ...
  async function upload(sb, file, progress, stage = () => {}, signal) {
    ...
    if (mediaType === 'video') {
      ...
      file = await root.CITLVideoConverter.convert(file, {stage:text=>stage(text,'prepare'), signal, layout:'native-cw'});
      await validateVideo(file);
      if(file.citlVideoLayout!=='native-cw')throw new Error('لم تُحمّل نسخة تجهيز Samsung الجديدة؛ حدّث صفحة الإدارة قبل رفع الفيديو.');
    ...
    row.video_layout=mediaType==='video'?(file.citlVideoLayout||'portrait'):null;
