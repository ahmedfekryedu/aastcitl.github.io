(function(){
  ...
  function printQrCards(){
    ...
    const termName=document.getElementById('qr-term')?.selectedOptions?.[0]?.textContent?.trim()||'الجدول الدراسي الحالي';
    const sheets=cards.map(card=>{
      ...
      return `<section class="sheet"><header><div class="college-name">كلية النقل الدولي واللوجستيات</div><img src="/CITL.png" alt="شعار كلية النقل الدولي واللوجستيات"></header><main><div class="room-heading"><span>القاعة</span><h1>${escapeHtml(roomName)}</h1></div><div class="qr-frame"><img class="qr" src="${qrImage}" alt="QR ${escapeHtml(roomName)}"></div><div class="faculty-note"><strong>تنبيه مهم</strong><span>هذا الرمز خاص بأعضاء هيئة التدريس فقط.</span></div></main><footer><span>${escapeHtml(termName)}</span><span>CITL Smart System</span></footer></section>`;
    }).join('');
    popup.document.write(`<!doctype html><html lang="ar" dir="rtl"><head><meta charset="utf-8"><title>QR القاعات</title><link href="https://fonts.googleapis.com/css2?family=Cairo:wght@400;700;900&display=swap" rel="stylesheet"><style>@page{size:A4 portrait;margin:0}*{box-sizing:border-box}html,body{margin:0;padding:0;background:#eef1f7;font-family:Cairo,Arial,sans-serif;color:#26316f}.sheet{width:210mm;height:297mm;margin:0 auto;background:#fff;padding:11mm 15mm 9mm;display:flex;flex-direction:column;page-break-after:always;break-after:page;overflow:hidden;position:relative}.sheet:before{content:'';position:absolute;top:0;right:0;left:0;height:4mm;background:#2A3475}.sheet:last-child{page-break-after:auto;break-after:auto}header{height:34mm;border-bottom:1.2mm solid #F3A628;display:flex;align-items:center;justify-content:space-between;padding:3mm 1mm 6mm;gap:8mm}header img{width:42mm;max-height:25mm;object-fit:contain;flex:none}.college-name{font-size:21px;font-weight:900;line-height:1.55;text-align:right;color:#2A3475}main{flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;padding:5mm 0}.room-heading{display:flex;align-items:center;justify-content:center;gap:4mm;margin-bottom:5mm}.room-heading span{display:inline-block;background:#F3A628;color:#2A3475;border-radius:999px;padding:1.8mm 7mm;font-size:11px;font-weight:900}.room-heading h1{font-size:34px;margin:0;font-weight:900;color:#2A3475}.qr-frame{width:112mm;height:112mm;margin:0 auto 7mm;padding:4mm;border:1px solid #d8deeb;border-radius:7mm;background:#fff;box-shadow:0 2mm 7mm rgba(42,52,117,.09)}.qr{display:block;width:100%;height:100%;object-fit:contain}.faculty-note{display:flex;align-items:center;justify-content:center;gap:3mm;width:min(145mm,100%);padding:3.5mm 6mm;border:1px solid #f2cf89;border-right:1.6mm solid #F3A628;border-radius:3mm;background:#fff9ec;color:#4b5563;font-size:11px;font-weight:700}.faculty-note strong{color:#2A3475;font-weight:900}footer{height:14mm;border-top:1px solid #e5e7eb;display:flex;align-items:flex-end;justify-content:space-between;color:#6b7280;font-size:8.5px;font-weight:700}@media screen{.sheet{margin:8mm auto;box-shadow:0 20px 60px rgba(42,52,117,.16)}}@media print{html,body{background:#fff}.sheet{margin:0;box-shadow:none}}</style></head><body>${sheets}<script>onload=()=>setTimeout(()=>print(),700)<\/script></body></html>`);
  ...
  document.getElementById('generate').addEventListener('click',async e=>{
    ...
    try{
      ...
      for(const q of r.generated){const card=document.createElement('article');card.className='qr-card';const h=document.createElement('h2');h.textContent=q.room_name;const canvas=document.createElement('canvas');const p=document.createElement('p');p.textContent='QR ثابت لهذه القاعة طوال الترم — خاص بأعضاء هيئة التدريس فقط';card.append(h,canvas,p);box.append(card);await window.QRCode.toCanvas(canvas,q.url,{width:260,margin:2,errorCorrectionLevel:'H'})}
