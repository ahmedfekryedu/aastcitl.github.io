...
function switchAdminTab(tabName) {
    ...
    if (tabName === 'news' && typeof loadNewsConfig === 'function') {
        ...
        // جلب عدد زيارات الامتحانات فور فتح التبويب
        try { fetchExamsVisitsCount(); } catch (e) { console.error(e); }
    }
...
// 2. حذف حجز فردي من اللوحة (تم التصحيح لتكون Global)
        window.deleteMeetingFromAdmin = async function(id) {
            if(!confirm('هل أنت متأكد من حذف هذا الحجز نهائياً من السجلات؟')) return;

            // تغيير شكل الزرار ليدل على التحميل (اختياري لو حابب تضيف ID للزرار)
            document.body.style.cursor = 'wait';
            ...
            try {
                ...
                updateAdminMeetingsList(); // تحديث الجدول في اللوحة
                renderCalendar();         // تحديث الجدول الرئيسي
                updateDashboard();        // تحديث الإحصائيات
                generateAISuggestions();  // تحديث المساعد الذكي
...
async function syncDashboardData() {
    // ---------------------------------------------------------
    // 1. التحديث الفوري للأرقام (من الذاكرة)
    // ---------------------------------------------------------
    const cachedHalls = localStorage.getItem('last_halls') || "0";
    const cachedMeetings = localStorage.getItem('last_meetings') || "0";
    const cachedLectures = localStorage.getItem('last_lectures') || "0";

    if(document.getElementById('count-halls')) document.getElementById('count-halls').innerText = cachedHalls;
    if(document.getElementById('count-meetings')) document.getElementById('count-meetings').innerText = cachedMeetings;
    if(document.getElementById('count-live-lectures')) document.getElementById('count-live-lectures').innerText = cachedLectures;

    // ---------------------------------------------------------
    // 2. تحديث شريط الأخبار (نطاق سرعة واسع جداً: سلحفاة 🐢 -> صاروخ 🚀)
    // ---------------------------------------------------------
    ...
    if (newsEl) {
        try {
            const { data } = await supabase
                ...
                .eq('setting_key', 'news_config')
                .maybeSingle();

            if (data && data.setting_value) {
                const newsData = data.setting_value;
                const newText = newsData.text || "";
                const currentText = newsEl.getAttribute('data-last-text');

                // التحديث لو النص اتغير أو الشريط فاضي
                if (currentText !== newText || newsEl.innerHTML.trim() === "") {
                    
                    const sentences = newText.split('\n').filter(s => s.trim() !== '');
                    const singleSet = sentences.join(' <span class="news-separator">✦</span> ');
                    
                    // وضع النص (نسخة واحدة)
                    newsEl.innerHTML = `<span>${singleSet}</span>`;
                    
                    newsEl.setAttribute('data-last-text', newText);

                    // ============================================================
                    // 🚀 المعادلة الجديدة (Wide Dynamic Range)
                    // ============================================================
                    
                    // 1. حساب المسافة (النص + الشاشة)
                    const textWidth = newsEl.scrollWidth; 
                    const containerWidth = newsEl.parentElement.offsetWidth || window.innerWidth;
                    const totalDistance = textWidth + containerWidth;

                    // 2. حساب السرعة (المعدلة لتكون محسوسة جداً)
                    const sliderVal = parseInt(newsData.sliderVal) || 50;
                    
                    // المعادلة القديمة كانت: 50 + (Val * 3) -> أقصى حاجة 350 (بطيء نسبياً)
                    // المعادلة الجديدة: تبدأ من 10 (زحف) وتصل إلى 1200+ (صاروخ)
                    const pixelsPerSecond = 35 + (sliderVal * 1.6);

                    // 3. الزمن
                    const duration = totalDistance / pixelsPerSecond;
                    
                    document.documentElement.style.setProperty('--ticker-speed', `${duration}s`);
                    
                    console.log(`🚀 سرعة: ${pixelsPerSecond}px/s | زمن الرحلة: ${duration.toFixed(1)}s`);
                }
            }
        } catch (e) {
            console.warn("⚠️ News Sync Error:", e);
        }
    }

    // ---------------------------------------------------------
    // 3. تحديث الإحصائيات (قاعات، حجوزات، محاضرات)
    // ---------------------------------------------------------
    try {
        // أ. عدد القاعات
        const { data: halls } = await supabase.from('academic_schedule').select('room_name');
        if (halls) {
            const uniqueHalls = [...new Set(halls.map(h => h.room_name))].length;
            if (uniqueHalls > 0 || halls.length === 0) {
                dashSetText('count-halls', uniqueHalls);
                localStorage.setItem('last_halls', uniqueHalls);
            }
        }

        // ب. اجتماعات المجلس
        const today = new Date().toISOString().split('T')[0];
        const { count: mCount, error: meetingsCountError } = await supabase.from('meetings').select('*', { count: 'exact', head: true }).eq('date', today).eq('status', 'confirmed');
        if (meetingsCountError) throw meetingsCountError;
        dashSetText('count-meetings', mCount || 0);
        localStorage.setItem('last_meetings', mCount || 0);
        ...
        const { data: lectures } = await supabase.from('academic_schedule').select('time_slot').eq('day_of_week', dayEn);
        if (lectures) {
            const activeCount = lectures.filter(item => {
                if (!item.time_slot) return false;
                const parts = item.time_slot.split(' - ');
                if (parts.length < 1) return false;
                const [h, m] = parts[0].split(':').map(Number);
                const startMins = h * 60 + (m || 0);
                return currentMins >= startMins && currentMins <= (startMins + 110);
            }).length;
            dashSetText('count-live-lectures', activeCount);
            localStorage.setItem('last_lectures', activeCount);
        }
    } catch (e) { 
...
// تشغيل الدالة
document.addEventListener('DOMContentLoaded', syncDashboardData);
setInterval(syncDashboardData, 60000);

// ==========================================
// 🚀 تحديث لحظي للسرعة (بنفس المعادلة القوية)
// ==========================================
document.addEventListener('DOMContentLoaded', () => {
supabase.channel('realtime-news')
    .on('postgres_changes', { event: '*', schema: 'public', table: 'site_settings' }, (payload) => {
        if (!payload.new || payload.new.setting_key !== 'news_config') return;
        ...
        if (!newConfig) return;
            ...
            if (newsEl) {
                ...
                // 2. تحديث السرعة لحظياً (المعادلة الجديدة)
                const sliderVal = parseInt(newConfig.sliderVal) || 50;
                
                // من 10 (سلحفاة) لـ 1200+ (صاروخ)
                const pixelsPerSecond = 35 + (sliderVal * 1.6);
                
                const contentWidth = newsEl.scrollWidth;
                const containerWidth = newsEl.parentElement.offsetWidth || window.innerWidth;
                const totalDistance = contentWidth + containerWidth;
                const duration = totalDistance / pixelsPerSecond;

                document.documentElement.style.setProperty('--ticker-speed', `${duration}s`);
                console.log(`🚀 تحديث لحظي: ${pixelsPerSecond} px/s`);
            }
        })
        .subscribe();
});
...
async function saveTvDisplayConfig() {
    ...
    const config = {
        ...
        examSlideDurationSeconds: examSeconds // 🔥 إضافة التوقيت المستقل
    };
    ...
    if (!error) showNotification('تم حفظ الإعدادات بنجاح', 'success');
}
...
async function saveNewsConfig() {
    ...
    localStorage.setItem(dbKey, JSON.stringify(config));
    ...
    if (!error) {
        ...
        console.error(error);
        showNotification('تم الحفظ محلياً فقط، تأكد من اتصال السيرفر المباشر', 'warning');
    }
}
...
document.addEventListener('change', (e) => {
    if (e.target && e.target.id === 'poster-file-input') {
        ...
        if (textEl) {
            textEl.textContent = e.target.files[0] ? e.target.files[0].name : 'اسحب الصورة هنا أو اضغط للتصفح';
        }
    }
});
...
async function loadTvPosterAdminData() {
    ...
    grid.innerHTML = '<div class="col-span-full text-center py-10 opacity-60 text-xs font-bold"><i class="fas fa-spinner fa-spin text-xl ml-2"></i>جاري استدعاء ألبوم البوسترات من Supabase...</div>';
            ...
            return `
                <div class="bg-white rounded-xl border border-gray-200 p-3 shadow-sm flex flex-col justify-between hover:shadow-md transition-shadow relative overflow-hidden">
                    <div class="flex gap-3">
                        <div class="w-16 h-20 rounded-lg bg-slate-900 border overflow-hidden shrink-0 flex items-center justify-center relative shadow-inner group">
                            <img src="${p.image_url}" class="w-full h-full object-contain" loading="lazy" />
                            <a href="${p.image_url}" target="_blank" class="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center text-white text-[10px]"><i class="fas fa-eye"></i></a>
                        </div>
                        <div class="min-w-0 flex-1 text-right">
                            <h5 class="font-black text-xs text-[#2A3475] truncate" title="${p.title || 'بدون عنوان'}">${p.title || 'إعلان بدون عنوان'}</h5>
                                ...
                                <div class="flex items-center gap-2 mt-1.5 pt-1.5 border-t border-gray-100">
                                    <input type="number" value="${p.duration || 15}" onchange="updatePosterQuickSettings('${p.id}', this.value, null)" class="w-12 text-center border border-gray-200 rounded text-[9px] py-0.5 focus:border-[#2A3475] outline-none" title="المدة (ثواني)"> ثانية
                ...
                <div class="flex items-center justify-between border-t border-gray-100 pt-2.5 mt-3">
                                        ...
                                        <div class="flex items-center gap-1">
                                            ...
                                            <button onclick="deleteTvPosterRecord('${p.id}', '${p.storage_path}')" class="w-7 h-7 bg-red-50 hover:bg-red-600 text-red-500 hover:text-white transition-colors rounded-lg flex items-center justify-center text-xs shadow-sm" title="حذف نهائي">
...
window.resetPosterForm = function() {
    ...
    document.getElementById('file-chosen-text').textContent = 'اسحب الصورة هنا أو اضغط للتصفح';
...
window.editTvPoster = function(id) {
    ...
    const p = window.adminLoadedPosters.find(x => x.id === id);
    if (!p) return;
    ...
    document.getElementById('file-chosen-text').textContent = 'اختر صورة جديدة (اختياري للاحتفاظ بالقديمة)';
...
// 3. دالة رفع البوستر ومعالجة الميديا عبر الـ Storage والـ Database معاً
...
if (posterForm) {
    posterForm.addEventListener('submit', async (e) => {
        ...
        const titleVal = document.getElementById('poster-title-input').value.trim();
        const order = parseInt(document.getElementById('poster-order-input').value, 10) || 1;
        const fitMode = document.getElementById('poster-fit-input').value;
        const durationVal = parseInt(document.getElementById('poster-duration-input').value) || 15;
        const playSoundVal = document.getElementById('poster-sound-input').checked;
        const editId = document.getElementById('poster-edit-id').value; // التقاط الـ ID

        // لو إحنا في وضع الإضافة، لازم نختار ملف
        if (!editId && (!fileInput.files || fileInput.files.length === 0)) {
            showNotification('يرجى تحديد ملف الصورة أولاً!', 'error');
            return;
        }

        showLoader(editId ? 'جاري حفظ التعديلات...' : 'جاري رفع ونشر البوستر...');

        try {
            let imageUrl = null;
            let storagePath = null;

            // إذا تم رفع صورة جديدة (سواء في التعديل أو الإضافة)
            if (fileInput.files && fileInput.files.length > 0) {
                ...
                const { error: uploadError } = await supabase.storage.from('tv-posters').upload(storagePath, file, { cacheControl: '3600', upsert: false });
                if (uploadError) throw uploadError;

                const { data: urlData } = supabase.storage.from('tv-posters').getPublicUrl(storagePath);
            ...
            if (editId) {
                // 🔥 وضع التعديل (UPDATE) 🔥
                let updates = {
                    title: titleVal, display_order: order, fit_mode: fitMode, 
                    duration: durationVal, play_sound: playSoundVal
                };
                // لو رفع صورة جديدة، حدث الروابط
                if (imageUrl) {
                    updates.image_url = imageUrl;
                    updates.storage_path = storagePath;
                }

                const { error: dbError } = await supabase.from('tv_posters').update(updates).eq('id', editId);
                if (dbError) throw dbError;

                showNotification('تم تحديث إعدادات البوستر بنجاح ✅');
            } else {
                // 🔥 وضع الإضافة (INSERT) 🔥
                const { error: dbError } = await supabase.from('tv_posters').insert([{
                    title: titleVal, image_url: imageUrl, storage_path: storagePath,
                    display_order: order, fit_mode: fitMode, duration: durationVal, play_sound: playSoundVal,
                    is_active: true, created_by: currentUser ? currentUser.full_name : 'إداري النظام'
                }]);
                if (dbError) throw dbError;
                showNotification('تم رفع ونشر البوستر الإعلاني بنجاح');
            }

            // تنظيف وإعادة التحميل
            resetPosterForm();
            await loadTvPosterAdminData();

        } catch (err) {
            console.error('Poster operation failed:', err);
            showNotification('فشل العملية: ' + (err.message || err), 'error');
        } finally {
            hideLoader();
        }
    });
...
async function deleteTvPosterRecord(id, storagePath) {
    const isConfirmed = await showConfirmDialog('هل أنت متأكد تماماً من حذف هذا البوستر الإعلاني نهائياً؟\nسيتم إزالته من شاشة العرض وحذف الصورة نهائياً من سيرفرات السحابة.');
    if (!isConfirmed) return;

    showLoader('جاري استئصال ملف الصورة وتنظيف ميتاداتا النظام...');

    try {
        // أولاً: تنظيف السحابة وإلغاء صورة الفايل الفيزيائي من الـ Storage
        if (storagePath) {
        ...
        // ثانياً: مسح سجل السطر الإعلاني نهائياً من الـ Database
        ...
        showNotification('تم استئصال وحذف البوستر نهائياً من شاشات الكلية');
        await loadTvPosterAdminData();

    } catch (err) {
        console.error('Poster deletion failed:', err);
        showNotification('حدث خلل أثناء محاولة الحذف: ' + err.message, 'error');
    } finally {
        hideLoader();
    }
}
...
async function saveTvPosterGlobalConfig() {
    ...
    try {
        ...
        if (error) throw error;
        showNotification('تم تحديث إعدادات تفضيلات البوسترات اللحظية');
    } catch (err) {
...
// ==========================================
// 🧠 نظام معالجة الإحصائيات المركزية (KPI Engine)
// ==========================================
async function updateExecutiveStats() {
    try {
        const now = new Date();
        ...
        // 1. جلب المحاضرات (للكارت الثاني)
        const { data: lectures } = await supabase.from('academic_schedule').select('time_slot, status').eq('day_of_week', dayName);
        
        let liveCount = 0;
        if (lectures) {
            lectures.forEach(item => {
                if (item.status === 'cancelled') return;
                // حساب وقت المحاضرة (نظام 110 دقيقة للفترة)
                const [h, m] = (item.time_slot || "00:00").split(' - ')[0].split(':').map(Number);
                const start = h * 60 + (m || 0);
                if (currentMinutes >= start && currentMinutes <= (start + 110)) liveCount++;
            });
        }
        dashSetText('stat-live-lectures', liveCount);

        // 2. جلب الحجوزات (للكارت الثالث)
        const { data: meetings, error: meetingsError } = await supabase.from('meetings').select('start_time').eq('date', todayDate).eq('status', 'confirmed');
        if (meetingsError) throw meetingsError;
        
        const mCount = meetings ? meetings.length : 0;
        dashSetText('stat-council-agenda', mCount);

        // إظهار موعد أول اجتماع قادم (ذكاء إضافي)
        if (mCount > 0) {
            ...
            if (upcoming) {
                document.getElementById('next-event-hint')?.classList.remove('hidden');
                dashSetText('next-time', upcoming.start_time);
            }
        }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    setInterval(updateExecutiveStats, 60000); // تحديث كل دقيقة
});

// دالة فتح الأجندة من كارت الإحصائيات الخارجي
window.openAgenda = function() {
    document.getElementById('admin-panel-modal').classList.add('show');
    switchAdminTab('meetings');
};

// دالة جلب ورسم الأجندة
...
function dashTimeToMinutes(timeStr) {
...
function dashScheduleStartMinutes(timeSlot) {
    if (!timeSlot) return null;
    const start = String(timeSlot).split(' - ')[0]?.trim();
    return dashTimeToMinutes(start);
}

function dashScheduleEndMinutes(timeSlot) {
    if (!timeSlot) return null;

    const parts = String(timeSlot).split(' - ');
    if (parts.length > 1) {
        const end = parts[1]?.trim();
        const endMins = dashTimeToMinutes(end);
        if (endMins !== null) return endMins;
    }

    const startMins = dashScheduleStartMinutes(timeSlot);
    if (startMins === null) return null;

    return startMins + 50;
}

function dashGetEnglishDayName() {
...
function dashMinutesToTime(totalMins) {
    if (typeof totalMins !== 'number' || Number.isNaN(totalMins)) return 'لا يوجد';

    const safeMins = ((totalMins % 1440) + 1440) % 1440;
    const hours = Math.floor(safeMins / 60).toString().padStart(2, '0');
    const minutes = (safeMins % 60).toString().padStart(2, '0');

    return `${hours}:${minutes}`;
}

function dashFindNextFreeRoom(liveLectures, currentMinutes) {
    const nextFree = (liveLectures || [])
        .map(r => ({
            room: r.room_name || 'قاعة',
            endMins: dashScheduleEndMinutes(r.time_slot)
        }))
        .filter(r => r.endMins !== null && r.endMins >= currentMinutes)
        .sort((a, b) => a.endMins - b.endMins)[0];

    return nextFree ? `${nextFree.room} - ${dashMinutesToTime(nextFree.endMins)}` : 'لا يوجد';
}

function dashCountReservationConflicts(rows) {
    const intervals = (rows || [])
        .map(r => {
            const start = dashTimeToMinutes(r.start_time);
            const end = dashTimeToMinutes(r.end_time);
            return {
                start,
    ...
    for (let i = 0; i < intervals.length; i++) {
        for (let j = i + 1; j < intervals.length; j++) {
            const isOverlap = intervals[j].start < intervals[i].end && intervals[j].end > intervals[i].start;
            if (isOverlap) conflicts++;
        }
    }
...
function dashFindAcademicDayEnd(todayLectures) {
    const latestEnd = (todayLectures || [])
        .map(r => dashScheduleEndMinutes(r.time_slot))
        .filter(v => v !== null)
        .sort((a, b) => b - a)[0];

    return latestEnd !== undefined
        ? `حتى ${dashMinutesToTime(latestEnd)}`
        : 'لا توجد محاضرات اليوم';
}

async function updateDashboardCardsOnly() {
    const db = getDashboardDbClient();
    const isManager = !!(currentUser && window.CITLPermissions.full(currentUser));

    const setLastUpdate = (text) => {
        dashSetText('dashboard-last-update', text || new Date().toLocaleTimeString('ar-EG', {
            hour: '2-digit',
            minute: '2-digit'
        }));
    };

    const applyPrivateVisibility = () => {
        document.querySelectorAll('[data-manager-only]').forEach(el => {
            el.classList.toggle('hidden', !isManager);
            el.setAttribute('aria-hidden', isManager ? 'false' : 'true');
        });
        ...
        // احتياطي لإخفاء كارت الحجوزات القديم لو موجود بدون data-manager-only
        const adminBookingNumber = document.getElementById('stat-admin-bookings');
        if (adminBookingNumber) {
            const adminCard = adminBookingNumber.closest('.group') || adminBookingNumber.closest('[class*="rounded"]');
            if (adminCard && !adminCard.hasAttribute('data-manager-only')) {
        ...
        // احتياطي لإخفاء بلوكات الأرقام الإدارية في الكروت القديمة
        const managerOnlyStatsIds = [
            'admin-bookings-live',
            'admin-bookings-next',
            'admin-bookings-pending',
    ...
    if (!db) {
        console.warn('Dashboard DB client not found.');
        dashSetText('academic-day-end', 'غير متصل');
        setLastUpdate('غير متصل');
        return;
    }
    ...
    const dayEn = typeof dashGetEnglishDayName === 'function'
        ? dashGetEnglishDayName()
    ...
    // =========================
    // 1) بيانات عامة: القاعات والجداول الأكاديمية
    // تظهر للجميع
    // =========================
    try {
        const { data: scheduleRows, error: scheduleError } = await db
            .from('academic_schedule')
            .select('room_name, day_of_week, time_slot, status');

        if (scheduleError) throw scheduleError;

        const allSchedule = scheduleRows || [];
        const activeSchedule = allSchedule.filter(r => !r.status || r.status === 'active');

        const allRooms = [...new Set(activeSchedule.map(r => r.room_name).filter(Boolean))];

        todayLectures = activeSchedule.filter(r => r.day_of_week === dayEn);

        liveLectures = todayLectures.filter(r => {
            const start = dashScheduleStartMinutes(r.time_slot);
            const end = dashScheduleEndMinutes(r.time_slot);
            return start !== null && end !== null && currentMinutes >= start && currentMinutes <= end;
        });

        const busyRoomsNow = [...new Set(liveLectures.map(r => r.room_name).filter(Boolean))];
        const availableRoomsNow = Math.max(allRooms.length - busyRoomsNow.length, 0);

        const nextLecture = todayLectures
            .map(r => ({
                ...r,
                startMins: dashScheduleStartMinutes(r.time_slot)
            }))
            .filter(r => r.startMins !== null && r.startMins > currentMinutes)
            .sort((a, b) => a.startMins - b.startMins)[0];

        const roomsUsagePercentage = allRooms.length
            ? Math.round((busyRoomsNow.length / allRooms.length) * 100)
            : 0;

        dashSetText('stat-total-rooms', allRooms.length);
        dashSetText('rooms-available-now', busyRoomsNow.length);
        dashSetText('rooms-busy-now', todayLectures.length);
        dashSetText('rooms-usage-percentage', `${roomsUsagePercentage}%`);

        dashSetText(
            'academic-day-status',
            dashFindAcademicOperationStatus(todayLectures, currentMinutes)
        );

        dashSetText('stat-live-lectures', liveLectures.length);
        dashSetText('lectures-today-count', todayLectures.length);
        dashSetText('lectures-next-time', nextLecture ? (nextLecture.time_slot || 'لا يوجد') : 'لا يوجد');

        // دي أرقام خاصة بالمديرين في كروت الجداول، لكن نحدثها فقط لو مدير
        if (isManager) {
            dashSetText('schedules-today-total', todayLectures.length);
            dashSetText('schedules-live-now', liveLectures.length);
        } else {
            dashSetText('schedules-today-total', 0);
            dashSetText('schedules-live-now', 0);
        }
    } catch (err) {
        console.error('Dashboard schedule block error:', err);

        dashSetText('stat-total-rooms', 0);
        dashSetText('rooms-available-now', 0);
        dashSetText('rooms-busy-now', 0);
        dashSetText('rooms-usage-percentage', '0%');
        dashSetText('academic-day-end', 'تعذر تحميل الجدول');

        dashSetText('stat-live-lectures', 0);
        dashSetText('lectures-today-count', 0);
        dashSetText('lectures-next-time', 'لا يوجد');
    ...
    // =========================
    // 2) بيانات خاصة: سجلات التواجد
    // للمديرين فقط
    // =========================
    if (isManager) {
        try {
            const { count: attendanceTodayCount, error: attendanceError } = await db
            ...
            const safeAttendanceCount = attendanceTodayCount || 0;

            dashSetText('schedules-attendance-today', safeAttendanceCount);
            dashSetText('lectures-attendance-today', safeAttendanceCount);
            dashSetText('lectures-without-attendance', Math.max(todayLectures.length - safeAttendanceCount, 0));
        } catch (err) {
            console.error('Dashboard attendance block error:', err);

            dashSetText('schedules-attendance-today', 0);
            dashSetText('lectures-attendance-today', 0);
            dashSetText('lectures-without-attendance', todayLectures.length || 0);
        }
    } else {
        dashSetText('schedules-attendance-today', 0);
        dashSetText('lectures-attendance-today', 0);
        dashSetText('lectures-without-attendance', 0);
    }

    // =========================
    // 3) بيانات خاصة: الحجوزات الإدارية
    // للمديرين فقط
    // مهم: غير المديرين لن يتم عمل query على بيانات الاجتماعات الإدارية
    // =========================
    if (!isManager) {
        dashSetText('stat-admin-bookings', 0);
        dashSetText('admin-bookings-live', 0);
        dashSetText('admin-bookings-next', 'لا يوجد');
        dashSetText('admin-bookings-pending', 0);
        dashSetText('admin-bookings-conflicts', 0);

        dashSetText('smrm-confirmed-today', 0);
        dashSetText('smrm-pending-today', 0);
        dashSetText('smrm-live-now', 0);
    ...
    try {
        ...
        const liveReservations = confirmedReservations.filter(r => {
            const start = dashTimeToMinutes(r.start_time);
            const end = dashTimeToMinutes(r.end_time);
            ...
            return currentMinutes >= start && currentMinutes <= (end ?? fallbackEnd);
        });

        const nextReservation = confirmedReservations
            .map(r => ({
                ...r,
                startMins: dashTimeToMinutes(r.start_time)
            }))
            .filter(r => r.startMins !== null && r.startMins > currentMinutes)
            .sort((a, b) => a.startMins - b.startMins)[0];

        dashSetText('stat-admin-bookings', confirmedReservations.length);
        dashSetText('admin-bookings-live', liveReservations.length);
        dashSetText('admin-bookings-next', nextReservation ? nextReservation.start_time : 'لا يوجد');
        dashSetText('admin-bookings-pending', pendingReservations.length);
        dashSetText('admin-bookings-conflicts', dashCountReservationConflicts(confirmedReservations));

        dashSetText('smrm-confirmed-today', confirmedReservations.length);
        dashSetText('smrm-pending-today', pendingReservations.length);
        dashSetText('smrm-live-now', liveReservations.length);
    } catch (err) {
        console.error('Dashboard reservations block error:', err);
...
window.addEventListener('load', () => {
    updateDashboardCardsOnly();
    setInterval(updateDashboardCardsOnly, 60000);
});

function dashFindAcademicOperationStatus(todayLectures, currentMinutes) {
    const ranges = (todayLectures || [])
        .map(r => {
            const start = dashScheduleStartMinutes(r.time_slot);
            const end = dashScheduleEndMinutes(r.time_slot);
    ...
    if (!ranges.length) return 'لا توجد محاضرات اليوم';

    const firstStart = ranges[0].start;
    const lastEnd = Math.max(...ranges.map(r => r.end));

    const liveNow = ranges.some(r =>
        currentMinutes >= r.start && currentMinutes < r.end
    );

    if (currentMinutes < firstStart) {
        return 'لم يبدأ التشغيل';
    }
...
async function fetchExamsVisitsCount() {
    ...
    try {
        ...
        const { count, error } = await window.supabaseAdmin
            .from('page_visits')
            .select('*', { count: 'exact', head: true });
