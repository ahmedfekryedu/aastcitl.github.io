...
for script in ["citl-runtime.js", "security-core.js", "registration-secure.js", "presence.js", "sw.js", "management/management.js", "attendance/attendance.js"]:
...
if migration.exists():
    ...
    for marker in [
        ...
        "qr_eligible_rooms",
        "term_visiting_lecturers",
        "visiting_lecturer_assignment",
    ]:
        if marker not in migration_text:
            errors.append(f"migration safety marker missing: {marker}")
    shortcut = root / "SUPABASE-UPDATE.sql"
    if (root / "SUPABASE-REPAIR.sql").read_bytes() != migration.read_bytes():
        errors.append("SUPABASE-REPAIR.sql differs from the canonical migration")
...
for marker in ["visitor-term", "visitor-name", "qr-summary", "eligible-rooms", "reports-term", "presence-sessions-list"]:
    if f'id="{marker}"' not in management_html:
        errors.append(f"management UI marker missing: {marker}")
for marker in ["state.schedule_instructors", "state.term_visitors", "state.qr_eligible_rooms", "QR ثابت لهذه القاعة طوال الترم"]:
...
if "join public.term_visiting_lecturers tv" not in sql_text:
    errors.append("visiting lecturers are not scoped to an academic term")

dashboard_html = (root / "dashboard" / "index.html").read_text(encoding="utf-8")
schedules_html = (root / "schedules" / "index.html").read_text(encoding="utf-8")
security_core = (root / "security-core.js").read_text(encoding="utf-8")
headers = (root / "cloudflare" / "security-headers.txt").read_text(encoding="utf-8")
if 'href="/management/"' in dashboard_html or 'href="/management/"' in schedules_html:
    errors.append("management link must not appear in dashboard or schedules headers")
for marker in [
    'id="schedule-presence-modal"',
    'function openSchedulePresenceModal()',
    'function amRenderActiveQrTokens()',
    "citl_generate_room_qr",
    'id="am-print-reports"',
]:
    if marker not in schedules_html:
        errors.append(f"native schedules attendance marker missing: {marker}")
if "/management/?embedded=1" in schedules_html:
    errors.append("schedules still embeds the obsolete management page")
if "location.href = '/management/" in security_core:
    errors.append("security core still redirects attendance management to a separate page")
if "table: 'citl_notifications'" not in security_core:
    errors.append("realtime notifications still target the legacy table")
if "X-Frame-Options: SAMEORIGIN" not in headers or "frame-ancestors 'self'" not in headers:
    errors.append("same-origin management popup is blocked by security headers")

if ".from('smrm_reservations')" in dashboard_html or '.from("smrm_reservations")' in dashboard_html:
    errors.append("dashboard still queries the obsolete smrm_reservations table")
if "if (!container) return;" not in dashboard_html:
    errors.append("dashboard AI suggestions do not guard a removed container")
if "@view-transition { navigation: auto; }" in dashboard_html or "@view-transition { navigation: auto; }" in (root / "smrm" / "index.html").read_text(encoding="utf-8"):
    errors.append("automatic cross-document transitions can produce unhandled AbortError")
if "as select date,start_time,end_time,status from public.meetings" not in sql_text:
    errors.append("public TV reservation view does not support the current meetings table")
