(function(root){
 ...
 async function save(){
  ...
  try{const result=await api(action,payload);if(action!=='archive.delete')reset();await load();message(action==='archive.delete'?'تم حذف نسخة الأرشيف المحددة نهائيًا. الجدول الحالي والحضور لم يتغيرا.':action==='clear'?'تم مسح الجدول الحالي للنوع المختار وحفظ نسخة أرشيفية. سجلات الحضور محفوظة.':action==='pause.save'?'تم تحديث فترة إيقاف الدراسة.':`تم الحفظ: ${result.count} صف؛ ${result.retained||0} دون تغيير، ${result.added||0} جديد أو معدل. النسخة السابقة في الأرشيف.`);if(action!=='archive.delete'){localStorage.setItem('citl-schedule-update',String(Date.now()));root.dispatchEvent(new Event('citl-schedule-update'));}}
