(function (root) {
  ...
  async function convert(file, {stage = () => {}, signal, layout = 'portrait'} = {}) {
    if(!['portrait','native-cw'].includes(layout))throw new Error('اتجاه تجهيز الفيديو غير صالح');
    ...
    try {
      ...
      const args = ['-hide_banner','-i','/input/source','-map',`0:${video.index}`,'-map','0:a:0?','-t',String(duration),
        ...
        '-vf',(source.format?.tags?.comment==='CITL_NATIVE_CW_V1'?'transpose=cclock,':'')+"scale=w='max(2,trunc(iw*sar/2)*2)':h='max(2,trunc(ih/2)*2)',setsar=1,scale=1080:1920:force_original_aspect_ratio=increase:force_divisible_by=2,crop=1080:1920,setsar=1,fps=30"+(layout==='native-cw'?',transpose=clock':''),
        ...
        '-map_metadata','-1','-map_chapters','-1',...(layout==='native-cw'?['-metadata','comment=CITL_NATIVE_CW_V1']:[]),'-movflags','+faststart','-threads','1','-y','output.mp4'];
      ...
      if (encoded?.codec_name !== 'h264' || encoded.width !== (layout==='native-cw'?1920:1080) || encoded.height !== (layout==='native-cw'?1080:1920) || encoded.pix_fmt !== 'yuv420p' || Math.abs(Number(output.format?.duration) - duration) > Math.max(1, duration * .01)) {
      ...
      const name = (file.name || 'video').replace(/\.[^.]+$/, '') + (layout==='native-cw'?'-tv-native-cw.mp4':'-tv.mp4');
      const result=new File([data], name, {type:'video/mp4'});
      result.citlVideoLayout=layout;return result;
    } catch (error) {
      if (signal?.aborted) throw cancelled();
