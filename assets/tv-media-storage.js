(function (root) {
  ...
  async function upload(sb, file, progress) {
    ...
    const row = {storage_provider:'r2', object_key:signed.objectKey, storage_path:signed.objectKey, image_url:signed.publicUrl, media_type:mediaType, mime_type:file.type, file_size:file.size, original_filename:file.name};
  ...
  async function save(sb, {old, metadata, file, progress = () => {}}) {
    if (!old && !file) throw new Error('اختر ملف الإعلان أولًا');
    const fresh = file ? await upload(sb, file, progress) : null;
    const id = old?.id;
  ...
  root.CITLMediaStorage = Object.freeze({validate, upload, save, remove, removeObject, isR2, putFile});
})(window);
