...
function updatePosterDurationControl() {
    ...
    const isVideo = file ? file.type === 'video/mp4' : row?.media_type === 'video' || /\.mp4(?:\?|$)/i.test(row?.image_url || '');
...
if (posterForm) posterForm.addEventListener('submit', async e => {
    ...
    const stage = text => { const el=document.getElementById('loader-text'); if(el)el.textContent=text; };
    ...
    try {
        ...
        const result = await window.CITLMediaStorage.save(supabase, {old,metadata,file,stage,progress:percent => {
