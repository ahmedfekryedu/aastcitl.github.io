...
async function updateAdminMeetingsList() {
            const body=document.getElementById('admin-meetings-list');
            if(body&&!window.CITLAdminMeetings)body.innerHTML=window.CITLLoading.table(6);
...
async function updateAdminUsersList() {
    ...
    activeUsersList.innerHTML = window.CITLLoading.table(6);
}
    }
...
async function renderTodayAgenda() {
    ...
    body.innerHTML = window.CITLLoading.table(3);
    ...
    try {
        ...
        `).join('');
    } catch (e) { console.error(e); body.innerHTML = '<tr><td colspan="3" class="text-center py-6 text-red-700">تعذر تحديث جدول المتابعة. أعد فتح التبويب للمحاولة.</td></tr>'; }
}
...
function pickRoomForEdit(id) {
    ...
    editingRoomId = r.id;
    window.CITLAcademicBooking.roomSelect(supabase,r.academic_room_name || '');
...
function resetRoomForm() {
    window.CITLAcademicBooking.roomSelect(supabase,'');
    editingRoomId = null;
...
async function saveRoom() {
    ...
    const academicSelect = document.getElementById('room-edit-academic');
    if (!academicSelect || academicSelect.disabled) return showNotification('انتظر تحميل قائمة ربط القاعات أولًا', 'error');
    const academic_room_name = academicSelect.value || null;

    if (!name) return showNotification('اكتب اسم القاعة', 'error');

    if (editingRoomId) {
        const { error } = await supabase.from('rooms')
            .update({ name, capacity, features, academic_room_name })
            .eq('id', editingRoomId);

        if (error) return showNotification('فشل التعديل', 'error');
    } else {
        const { error } = await supabase.from('rooms')
            .insert([{ name, capacity, features, academic_room_name }]);
    ...
    window.CITLAcademicBooking.invalidate();
    await loadMeetings();
...
window.approveMeeting = async function(id) {
 let meetings = window.CITLAdminMeetings || [];
    ...
    try {
        ...
        const {data:record,error:readError} = await supabase.from('meetings').select('*').eq('id',id).single();
        if(readError) throw readError;
        const override = await window.CITLAcademicBooking.check(supabase,record,currentUser);
        const { error } = await supabase.from('meetings').update({ status: 'confirmed', ...override }).eq('id', id);
        if(error) throw error;
        ...
        console.error(err);
        showNotification(err.message || 'خطأ في الاعتماد', 'error');
    } finally {
        document.body.style.cursor = 'default';
    }
};
...
window.approveModification = async function(id) {
 let meetings = window.CITLAdminMeetings || [];
    ...
    try {
        ...
        const override = await window.CITLAcademicBooking.check(supabase,{...meeting,...changes},currentUser);
        const { error } = await supabase.from('meetings').update({
            ...
            status: 'confirmed', pending_changes: null, ...override
        }).eq('id', id);
        if(error) throw error;
        ...
        await loadMeetings(); 
    } catch(err) { console.error(err); showNotification(err.message || 'خطأ في العملية', 'error'); }
