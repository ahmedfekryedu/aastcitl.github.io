(function(root){
  ...
  function status(){
    ...
    return `<div class="study-sync" role="status">${lastGood?'الاتصال متوقف · آخر تحديث اليوم '+M.escape(time):'تعذر تحميل جدول اليوم · نحاول استعادة الاتصال'}</div>`;
  }
  ...
  function renderRoom(page,container){
    ...
    container.innerHTML=`<section class="room-card study-screen">
    ...
    if(root.QRCode)root.QRCode.toCanvas(canvas,url.href,{width:132,margin:4,errorCorrectionLevel:'M',color:{dark:'#101630',light:'#ffffff'}},error=>{if(error){const fallback=document.createElement('span');fallback.className='study-qr-error';fallback.textContent='تعذر تحميل الرمز';canvas.replaceWith(fallback);}});
  }
