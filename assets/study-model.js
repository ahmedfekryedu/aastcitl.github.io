(function(root){
  ...
  function runsOn(row,date){return !(row.study_pauses||[]).some(p=>date>=p.starts_on&&date<=p.ends_on)&&(!row.term_id||(row.term_active===true&&!!date&&date>=row.term_starts_on&&date>=(row.effective_from||row.term_starts_on)&&date<=row.term_ends_on));}
