(function(root){
  ...
  function updateNews(config){root.CITLTicker.apply(config||{text:'أهلًا بكم في نظام الكلية'},'news_config');}
  ...
  function renderOverview(items,stats,now){
    ...
    const rooms=summaryCard('إشغال القاعات','إجمالي القاعات',failed?'—':stats.rooms.length,[metric('قاعات بلا محاضرات',failed?'—':available),metric('مشغولة الآن',failed?'—':busy),metric('نسبة الإشغال',failed?'—':usage+'%')],roomDetails);
    ...
    const lectures=summaryCard('الحركة الدراسية','محاضرات اليوم',failed?'—':stats.today.length,[metric('محاضرات جارية',failed?'—':stats.live.length),metric('انتهت',failed?'—':ended),metric('لم تبدأ',failed?'—':upcoming)],lectureFooter,'dh-lecture-summary');
    let administration='';
    if(r.meetings){
  ...
  async function refresh(){
    ...
    pending=(async()=>{
      $('dh-refresh').disabled=true;$('dashboard-home').setAttribute('aria-busy','true');
      let profile;
      ...
      if(!state.ready||oldKey!==newKey){$('dh-loading').innerHTML=root.CITLLoading.summary()+(M.role(profile).faculty?'<table style="width:100%"><tbody>'+root.CITLLoading.table(5)+'</tbody></table>':'');$('dh-loading').hidden=false;}
    ...
    })().finally(()=>{pending=null;$('dh-refresh').disabled=false;$('dashboard-home').setAttribute('aria-busy','false');});
    return pending;
  }
  ...
  function start(){
    ...
    ['academic_schedule','schedule_actions','profiles','meetings'].forEach(table=>channel.on('postgres_changes',{event:'*',schema:'public',table},requestRefresh));channel.subscribe();
    root.CITLTicker.watch(root.sb,()=> 'news_config');
    return refresh();
  }
