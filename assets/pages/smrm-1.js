...
// =======================================================
// 🔥🔥 أهم خطوة: ربط العملاء بالنافذة (Global Exposure) 🔥🔥
// =======================================================
window.supabase = sb;
...
function applyUserPermissions() {
    ...
    if(mobName) mobName.textContent = currentUser.full_name || 'مستخدم';
    if(mobRole) {
    ...
    // عناصر الكبسولة الجديدة (بدلاً من user-welcome)
    const profileName = document.getElementById('profile-name');
    const profileRole = document.getElementById('profile-role');

    // 1. عرض بيانات المستخدم (الاسم والوظيفة) في الكبسولة
    if(profileName && typeof currentUser !== 'undefined' && currentUser) {
        ...
        // تعبئة البيانات
        // السطر الأول: الاسم
        profileName.textContent = `مرحباً، ${currentUser.full_name || currentUser.email}`;
    ...
    // 2. التحكم في زر لوحة التحكم
    if (adminBtn) {
        // الزر يظهر للجميع الآن
        adminBtn.style.display = 'flex';
        
        // نغير النص والأيقونة حسب الدور
        ...
        if (window.CITLPermissions.full(currentUser)) {
            // 🔥 التعديل المطلوب: لوحة التحكم 🔥
            if(btnText) btnText.textContent = 'لوحة التحكم'; 
            if(btnIcon) btnIcon.className = 'fas fa-cogs text-[#F3A628]'; 
            
            // (زر المدير يعمل بشكل افتراضي لفتح المودال)
        } else {
            // للمستخدم العادي
            if(btnText) btnText.textContent = 'ملفي الشخصي';
            if(btnIcon) btnIcon.className = 'fas fa-user-circle text-[#F3A628]';
            
            // تحويل الزر لفتح الملف الشخصي عند الضغط
    ...
    // 3. زر الخروج
// 3. زر الخروج - نسخة معدلة تضمن العودة الصحيحة وتنظيف الجلسة
if(logoutBtn) {
    // حذف أي "Listeners" قديمة عشان الزرار ميكررش الأوامر
    const newBtn = logoutBtn.cloneNode(true);
    logoutBtn.parentNode.replaceChild(newBtn, logoutBtn);
    
    newBtn.addEventListener('click', async (e) => {
        e.preventDefault(); // منع المتصفح من تنفيذ أي أمر افتراضي
        
        try { 
            // تسجيل الخروج من سيرفر Supabase (مهم جداً عشان خاصية تذكرني)
            if (typeof supabase !== 'undefined') {
        ...
        // مسح كل البيانات المحلية (التوكن، بيانات المستخدم، إلخ)
        localStorage.clear();
        
        // العودة لصفحة تسجيل الدخول (استخدام ../ أضمن بكتير من /)
...
const defaultConfig = {
    system_title: "التحكم وإدارة القاعات الدراسية - CITL Smart System",
    college_name: "كلية النقل الدولي واللوجستيات",
    room_name: "القاعة الحالية"
};
...
async function initializeApp() {
    try {
        ...
        // ---------------------------------------------------------
        // 🔥 تشغيل نظام التواجد Presence لـ SMRM
        // ---------------------------------------------------------
        if (window.presenceTracker) {
            try {
                window.presenceTracker.startForPage('smrm').catch(error => console.warn('Presence unavailable:', error));
            } catch (presenceError) {
                console.error('Presence SMRM start failed:', presenceError);
            }
        }
        ...
        if (window.elementSdk) {
            await window.elementSdk.init({
                ...
                onConfigChange: async (config) => {
                    ...
                    if (roomNameElement && !currentRoom) roomNameElement.textContent = config.room_name || defaultConfig.room_name;
                },
        ...
        // 2. تحميل القاعات واختيار القاعة الحالية
        await loadRooms();

        const hasSaved = restoreSelectedRoom();

        if (!hasSaved) {
            // اختار افتراضي: حاول قاعة مجلس/اجتماعات مجلس.. لو مش موجودة خُد أول قاعة
            const preferred =
                rooms.find(r => (r.name || '').includes('مجلس')) ||
                rooms.find(r => (r.name || '').includes('اجتماعات')) ||
                rooms[0] ||
                null;

            if (preferred) {
                currentRoomId = preferred.id;
                currentRoom = preferred;
                localStorage.setItem(ROOM_STORAGE_KEY, preferred.id);
                updateRoomHeaderUI();
            }
        }

        // 3. تحميل الحجوزات من السيرفر (للقاعة المختارة فقط)
        await loadMeetings();

        // 4. تشغيل الجدول والواجهة
        initializeCalendar();
        setupEventListeners();
        populateTimeSlots();
...
// ============================================================
// تحديث قائمة المستخدمين وإحصائياتهم (النسخة النهائية الذكية + Presence)
// ============================================================
...
async function loadTvDisplayConfig() {
    try {
        ...
        if (config) {
            ...
            if (Array.isArray(config.activeRooms) && config.activeRooms.length > 0) {
...
async function saveTvDisplayConfig() {
    ...
    if (selectedRooms.length === 0) {
        showNotification('لازم تختار قاعة واحدة على الأقل لشاشة العرض', 'error');
        return;
    }

    const secondsInput = document.getElementById('tv-slide-seconds');
    const slideDurationSeconds = clampTvSeconds(secondsInput ? secondsInput.value : 10);
    ...
    localStorage.setItem('tv_display_config', JSON.stringify(config));
    ...
    if (error) {
        console.error(error);
        showNotification('تعذر حفظ إعدادات الشاشة، راجع الاتصال أو صلاحيات Supabase', 'error');
        return;
    }

    tvActiveRooms = selectedRooms;

    showNotification('تم حفظ إعدادات شاشة العرض ✅', 'success');
}
...
// 1. دالة الحفظ (تكتب في الجدول الجديد)
async function saveNewsConfig() {
    const text = document.getElementById('news-input-multi').value;
    const sliderVal = document.getElementById('news-speed-slider').value;
    const calculatedSpeed = null;

    if (!text.trim()) {
        alert('اكتب نص للخبر الأول!');
        return;
    }
    ...
    const { error } = await supabase
        .from('site_settings')
        .upsert({ 
            setting_key: 'news_config', 
            setting_value: config 
        }, { onConflict: 'setting_key' });

    if (!error) {
        showNotification('تم الحفظ وتحديث الشريط ✅');
    } else {
        console.error(error);
        showNotification('تم الحفظ محلياً فقط (تأكد من النت)', 'warning');
    }
}
...
if (posterForm) {
    posterForm.addEventListener('submit', async (e) => {
        ...
        const orderInput = document.getElementById('poster-order-input');
        const fitInput = document.getElementById('poster-fit-input');
        ...
        // إظهار اللودر المركزي للأكاديمية مع نص واضح لموظف التلفزيون
        showLoader('جاري ضغط ونشر البوستر في الـ Storage Bucket...');

        try {
            // أ. تكوين مسار فريد للملف داخل الـ Storage لمنع التكرار والتعارض باستخدام الطابع الزمني
            const fileExt = file.name.split('.').pop();
            const fileName = `${Date.now()}_${Math.random().toString(36).substring(2, 7)}.${fileExt}`;
            const storagePath = `posters/${fileName}`;

            // ب. رفع الصورة الفعلية إلى الـ Bucket المستهدف
            ...
            // ج. استخراج الرابط العام لظهور البوستر الفوري
            ...
            // د. إدخال السجل بجدول الميتاداتا ليلقطه تلفزيون الكلية فوراً بـ Realtime
            const { error: dbError } = await supabase
                ...
                .insert([{
                    ...
                    fit_mode: fitMode,
                    is_active: true,
                    created_by: currentUser ? currentUser.full_name : 'إداري النظام'
                }]);
            ...
            // هـ. تنظيف نموذج الرفع وإعادة تهيئة نصوص الملفات المخصصة
            posterForm.reset();
            const textEl = document.getElementById('file-chosen-text');
            if (textEl) textEl.textContent = 'اسحب الصورة هنا أو اضغط للتصفح';
            
            // و. تحديث العرض للأدمن فوراً
            await loadTvPosterAdminData();

        } catch (err) {
            console.error('Poster upload failed:', err);
            showNotification('فشل رفع ونشر البوستر: ' + (err.message || err), 'error');
        } finally {
            hideLoader();
        }
    });
...
// 2. دالة التحميل (تقرأ من الجدول الجديد عند فتح الصفحة)
async function loadCurrentNews() {
    ...
    if (data && data.setting_value) {
        ...
        if(document.getElementById('news-input-multi')) 
            document.getElementById('news-input-multi').value = config.text || '';
        
        if(document.getElementById('news-speed-slider')) {
            document.getElementById('news-speed-slider').value = config.sliderVal || 50;
            if (typeof updateSpeedLabel === "function") updateSpeedLabel(config.sliderVal || 50);
        }
