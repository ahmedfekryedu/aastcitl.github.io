(function(){
  ...
  const labels={paused:'متوقفة مؤقتًا',out_of_term:'خارج فترة الدراسة',live:'جارية الآن',next:'قادمة اليوم',cancelled:'ملغاة',scheduled:'مجدولة',ended:'انتهت اليوم'};
  function updateDate(){const now=new Date();$('study-date').innerHTML=`<span>${new Intl.DateTimeFormat('ar-EG',{weekday:'long',timeZone:'Africa/Cairo'}).format(now)}</span><span>${new Intl.DateTimeFormat('en-GB',{day:'2-digit',month:'short',timeZone:'Africa/Cairo'}).format(now)}</span>`;$('study-year').textContent=now.getFullYear();}
  function render(){
